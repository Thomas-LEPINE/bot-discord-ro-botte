    ...
    if(["eightball", "🍪", "👌"].includes(emoji) && message.channel.id === channel.id) {
        switch (emoji) {
            case "eighball":
                ...
                message.channel.send(`Le rôle ${eightballisteRole} a été supprimé à ${user.id}`);
                break;
            case "🍪":
                member.roles.remove(cookierRole);
                message.channel.send(`Le rôle ${cookierRole} a été supprimé à ${user.id}`);
                break;
            case "👌":
                member.roles.remove(okManRole);
                message.channel.send(`Le rôle ${okManRole} a été supprimé à ${user.id}`);
                break;
        }
    } else {
        return;
